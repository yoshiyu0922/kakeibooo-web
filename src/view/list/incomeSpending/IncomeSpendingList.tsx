...
const IncomeSpendingList: React.FC<Props> = props => {
  ...
  const confirmDelete = (id: number, data: IncomeSpendingType) => {
    confirm({
      ...
      onOk() {
        const repository = Repository.instance;
        repository.deleteIncomeSpend(id,
          (_: AxiosResponse) => {
            Modal.info({title: '削除成功', content: '削除に成功しました'});
            const updated = list.filter((i: IncomeSpendingType) => i.incomeSpendingId !== id);
            setList(updated);
          },
          () => {
  ...
  const columns = [
    {
      ...
      render: (id: number, data: IncomeSpendingType) => {
        if (props.isMain) {
          ...
          return (<Button size="small" onClick={() => confirmDelete(id, data)}><Icon type="delete"/></Button>);
        }
      },
      width: 30
    },
    ...
    },
    {
      ...
      render: (v: number, record: IncomeSpendingType) => {
        ...
        return (<span> {amount.toLocaleString()}円<br/>({record.howToPayName}) </span>)
      },
      width: 100,
    ...
    },
    {
      ...
      render: (name: string, record: IncomeSpendingType) => {
        return (
          <span>{record.parentCategoryName} > {name}</span>
        )
      },
      width: 210
    },
  ...
  return (
    <div className={styles.list}>
      ...
      <Table pagination={{pageSize: props.offset}} rowKey={"incomeSpendingId"} columns={columns} dataSource={list}
             scroll={{x: 1200}}/>
